    ...
    <div className="builder-canvas flex relative flex-wrap gap-2.5 mt-7 min-h-[323px] max-md:max-w-full">
      {features.map((feature, index) => (
        <React.Fragment key={index}>
          ...
          />
          <div className="absolute text-2xl text-slate-700 left-[53px]" style={{ top: `${30 + index * 57}px` }}>
            {feature}
          </div>
        </React.Fragment>
      ))}
      <button className="flex absolute flex-col justify-center self-center px-14 py-2.5 text-4xl font-bold text-orange-50 bg-red-600 rounded-3xl h-[62px] left-[280px] top-[237px] w-[300px] max-md:px-5">
